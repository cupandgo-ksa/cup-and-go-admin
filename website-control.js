...
function ensureWebsiteBrandGalleryFields(){
  ...
  box.innerHTML=
    ...
    '<div id="website-store-gallery-fields" style="display:grid;gap:10px;"></div>'+'<div style="font-size:10px;font-weight:900;color:var(--orange);margin:18px 0 9px;">Signboard Artwork — Full Image</div>'+'<div style="display:grid;grid-template-columns:95px 1fr;gap:10px;align-items:center;padding:9px;border-radius:14px;border:1px solid var(--glass-border);background:rgba(255,255,255,.035);"><div style="height:86px;border-radius:12px;overflow:hidden;background:rgba(255,255,255,.04);display:grid;place-items:center;"><img id="website-signboard-preview" style="width:100%;height:100%;object-fit:contain;display:none"><span id="website-signboard-empty" style="font-size:9px;opacity:.5;">Signboard</span></div><div><label class="btn" style="display:flex;align-items:center;justify-content:center;padding:10px;border-radius:11px;cursor:pointer;background:rgba(255,255,255,.08);color:var(--orange);">🪧 Upload full signboard image<input type="file" accept="image/*" style="display:none" onchange="uploadWebsiteSignboardImage(event)"></label><button type="button" class="btn" onclick="removeWebsiteSignboardImage()" style="width:100%;margin-top:6px;padding:8px;border-radius:11px;background:rgba(255,59,48,.1);color:#ff8179;">Remove</button><input id="website-signboard-image-url" type="hidden"></div></div>'+
